...
fn main() -> Result<(), Error> {
    ...
    let matches = App::new("The Polymesh Compliance Token Tool")
        ...
        .subcommand(
            ...
            .arg(
                Arg::with_name("key")
                .help("The private key (hex form) to use for creating the compliance token. A new one is generated by default.")
                .short("k")
                .long("key").takes_value(true).value_name("KEY_IN_HEX"),
                ),
                )
        .get_matches();
...
fn handle_gen(matches: &ArgMatches) -> Result<(), Error> {
    ...
    // Use an array to enforce a rigid random key size
    let mut secret_key_arr = [0u8; SECRET_KEY_SIZE];

    // TODO: Use the option as a filename to avoid revealing the key in terminal
    if let Some(secret_key_string) = matches.value_of("key") {
        secret_key_arr.copy_from_slice(hex::decode(secret_key_string)?.as_slice());
    } else {
        info!("Generating a new secret key");
        secret_key_arr = rand::random();
    }
...
fn handle_transfer(secret_key: &SecretKey) -> Result<(), Error> {
    ...
    // Sign the computed hash
    let mut signature = secp.sign(&Message::from_slice(&hash)?, secret_key);
